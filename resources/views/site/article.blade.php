...
@section('breadcrumb-content')
    @if($postRep->region)
        <li><a href="{{route('site.section', ['region' => $postRep->region->slug])}}">{{ $postRep->region->name }}</a></li>
    @endif
    @if($postRep->city)
        <li><a href="{{route('site.section', ['region' => $postRep->city->slug])}}">{{ $postRep->city->name }}</a></li>
    @endif
            ...
            <div class="product-tag blog-tag mt-3">
                <ul>
                    ...
                    @if(count($post_info->tags) > 0)
                        ...
                        <li><a href="{{ route('site.section.tag', ['region' => $post_info->countri->slug, 'tag' => $tag->slug]) }}">{{ $tag->title }}</a></li>
                        @endforeach
                    @endif
                </ul>
            </div>
